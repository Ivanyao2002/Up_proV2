...
import type { SosDashboard } from "../api/sos.types";
import type { SosPortalRoutes } from "../lib/sosPortalConfig";
import { SosActiveIncidentCard } from "./SosActiveIncidentCard";

interface SosGuardianViewProps {
...
export function SosGuardianView({
  routes,
  breadcrumb,
  subtitle = "Surveillance temps réel des incidents SOS clients et chauffeurs. Rafraîchissement automatique toutes les 30 secondes.",
  data,
  isLoading,
  isError,
  dataUpdatedAt,
  isFetching,
}: SosGuardianViewProps) {
  if (isError) {
    return (
  ...
  return (
    <div className="animate-fade-up">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {active.map((incident) => (
            <SosActiveIncidentCard
              key={incident.id}
              incident={incident}
              detailHref={routes.incidentDetail(incident.id)}
            />
          ))}
        </div>
      )}
