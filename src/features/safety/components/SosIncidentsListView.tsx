...
type UseSosIncidentsList = (
  params?: SosListParams
) => UseQueryResult<Paginated<SosIncident>>;

interface SosIncidentsListViewProps {
  routes: SosPortalRoutes;
  breadcrumb: string[];
  useIncidentsList: UseSosIncidentsList;
}

export function SosIncidentsListView({
  routes,
  breadcrumb,
  useIncidentsList,
}: SosIncidentsListViewProps) {
  ...
  const { data, isLoading, isError } = useIncidentsList(table.listParams);
  ...
  const columns: Column<SosIncident>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <Link
          href={routes.incidentDetail(row.id)}
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader
        ...
        breadcrumb={[...breadcrumb, "Incidents"]}
        actions={
          <Link
