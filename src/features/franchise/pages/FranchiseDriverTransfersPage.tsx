...
export function FranchiseDriverTransfersPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useFranchiseDriverRechargeStats();
  const { data, isLoading, isError } = useFranchiseDriverTransfers(table.listParams);
  ...
  const columns: Column<PartnerDriverTransfer>[] = [
    ...
    },
    {
      ...
      cell: (t) => (
        <div>
          <Link
            href={`/franchise/drivers/${t.driver_id}`}
  ...
  return (
    <div className="animate-fade-up">
      ...
      <p className="mb-6 max-w-2xl text-sm text-muted">
        ...
        vos chauffeurs. Historique des montants dépensés et statut de crédit
        sur l&apos;application.
      </p>
      ...
      {statsLoading ? (
        ...
        <div className="animate-stagger mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <KpiCard label="Transferts" value={String(stats.transfers_count)} />
          <KpiCard
            label="Ce mois"
      ...
      <TableFiltersBar
        ...
        searchPlaceholder="Réf., chauffeur…"
      ...
      {isError ? (
        ...
        <DataTable
          ...
          exportFileName="recharges-chauffeurs-franchise"
