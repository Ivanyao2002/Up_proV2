...
export function PartnerFreightDetailPage() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: string; status: "accepted" | "rejected" } | null>(null);
    ...
    <div className="animate-fade-up pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Suivi avancé (assignation véhicule, transit, POD, tracking) :
              en attente des endpoints backend sous scope partenaire (cf. demande DB-02). */}
          {offer.status === "accepted" && (
            <div className="rounded-card bg-surface p-6 shadow-card">
              <h2 className="text-lg font-semibold mb-2">Suivi de la livraison</h2>
              <p className="text-sm text-muted">
                L&apos;assignation de véhicule, le suivi de transit et la preuve de livraison (POD)
                seront disponibles dès la livraison des endpoints backend correspondants.
              </p>
            </div>
          )}
        </div>
