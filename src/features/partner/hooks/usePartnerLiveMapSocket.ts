...
export function usePartnerLiveMapSocket(
  ...
  useEffect(() => {
    ...
    if (!config?.url || !config.event) {
      setStatus("idle");
      return;
    }

    const socketUrl = normalizeSocketIoUrl(config.url);
    ...
    const onConnect = () => {
      setStatus("connected");
      if (config.joinPayload) {
        socket.emit("join", config.joinPayload);
      }
    };
    ...
    socket.on(config.event, onLocations);
    ...
    return () => {
      ...
      socket.off(config.event, onLocations);
