...
import {
  useChatConversation,
  useSendChatMessage,
} from "../api/chatConversations.queries";

interface ChatConversationDetailPageProps {
  conversationId: string;
}

export function ChatConversationDetailPage({ conversationId }: ChatConversationDetailPageProps) {
      ...
      <div className="flex min-h-[420px] flex-col rounded-card border border-border bg-surface shadow-card">
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {data.messages.length === 0 ? (
            ...
            data.messages.map((m) => {
              const isAdmin = m.author_role === "admin" || m.author_role === "agent";
              return (
                <div key={m.id} className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${
                      isAdmin ? "bg-teal text-white" : "bg-canvas text-foreground"
                    }`}
                  >
                    <p className={`mb-1 text-xs font-medium ${isAdmin ? "text-teal-100" : "text-muted"}`}>
                      {m.author_name}
                    </p>
                    <p className="whitespace-pre-wrap">{m.body}</p>
                    <p className={`mt-1 text-[10px] ${isAdmin ? "text-teal-100/80" : "text-muted"}`}>
