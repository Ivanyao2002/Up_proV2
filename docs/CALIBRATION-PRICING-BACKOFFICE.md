...
prix_base (palier de distance)
   × multiplicateur trafic/météo (contexte)
...
Deux autres jeux de données **adjacents** influencent le prix mais ne sont **pas** dans `pricing.config` (voir §8) :
...
| `trafficPolicy` | objet | voir §7.4 | — | Heures de pointe, baseline urbaine, inférence trafic. |
...
7. **Trafic** : niveau (auto/défaut → profil de pointe matchant → inférence offre/demande) ; `durationMultiplier` (baseline urbaine + profil, plafonné `maxDurationMultiplier`, =1 si durée temps réel) ; `priceTrafficMult`.
...
13. **Multiplicateur combiné** = `min( contexte × surge , priceCapGlobal )` — *le plafond s'applique au **produit**, pas à chaque facteur, pour éviter un cap²*.
...
- **Toujours valider** après calibration : relire `effective` via `GET /v1/admin/pricing-config?countryCode=CI` et comparer un devis réel.
...
- `src/modules/pricing/pricing.traffic.ts` — trafic/heures de pointe
...
- `src/modules/admin/admin.routes.ts` (≈ l.1193–1235) — routes admin
</content>
</invoke>
