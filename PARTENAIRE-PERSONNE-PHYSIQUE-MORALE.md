...
// Corps
{
  ...
  "managerFirstName": "Amadou",   // gérant — seulement si COMPANY (cf. §4.6)
  "managerLastName": "Diallo",    // gérant — seulement si COMPANY
...
### 4.6 Nom du gérant / représentant légal (personne morale)

> Statut : **DÉPLOYÉ LIVE 2026-06-22** (migration 115). Validé E2E (17/17).

Pour une **personne morale**, `legal_name` porte la **raison sociale** (la société). Le **gérant**
(la personne physique qui la représente) se saisit désormais explicitement à la création **et** en
modification, sans attendre l'upload de la pièce `MANAGER_ID_CARD` (qui reste la preuve documentaire).

| Champ entrée (accepté camelCase **et** snake_case) | Rôle |
|----------------------------------------------------|------|
| `managerFirstName` / `manager_first_name` | Prénom du gérant |
| `managerLastName` / `manager_last_name`   | Nom du gérant |
| `managerFullName` / `manager_full_name` (alias `manager_name`) | Nom complet une ligne (scindé sur le 1er espace si prénom/nom non séparés) |

**Règles :**

- Champs persistés **uniquement** si `legal_form = COMPANY`. Pour `INDIVIDUAL`, ils sont **ignorés
  silencieusement** (pas d'erreur — le partenaire *est* la personne). Repasser un partenaire en
  `INDIVIDUAL` via PATCH **purge** le gérant.
- En **PATCH**, seuls les champs gérant **réellement envoyés** sont écrits : envoyer uniquement
  `managerLastName` met à jour le nom **sans effacer** le prénom (changement de gérant partiel).
- **Repli** : si `legal_form = COMPANY` et qu'aucun `manager_*` n'est fourni mais que `firstName` /
  `lastName` le sont (ce que le formulaire actuel envoie déjà), ils sont recopiés vers le gérant.
- En lecture, l'API expose `manager_first_name`, `manager_last_name` et un champ calculé
  **`manager_display_name`** (« Prénom Nom », ou `null`).
- La **recherche** admin (`GET /v1/admin/partners?search=`) couvre désormais le nom du gérant.

```jsonc
// PATCH /v1/admin/partners/:id — changer le gérant sans recréer le partenaire
{ "managerFirstName": "Amadou", "managerLastName": "Traoré" }
```

> Il n'y a **pas** de nouveau code d'erreur : `managerFirstName`/`managerLastName` sont optionnels
> (la pièce `MANAGER_ID_CARD` reste, elle, requise pour un dossier morale **complet**).
...
- **Migration** : `migrations/115_partners_manager_name.sql` (additive, idempotente)
  - `partners.manager_first_name` / `partners.manager_last_name` (nullables) + index `lower(...)` pour la recherche.
  - Helper `src/shared/partner-manager.ts` (`parseManagerName`, `managerPatch`, `managerDisplayName`).
  - Câblé dans `partners.service.ts` (create/update/detail self-service), `admin.lists.service.ts`
    (liste + détail + patch + recherche), `franchises.service.ts` (liste + détail).
- **Code** :
...
2. Si **morale** : ajouter un bloc **« Gérant »** (`managerFirstName` / `managerLastName`) au formulaire ; remplacer l'usage ambigu de `firstName`/`lastName` (qui sert au compte portail). Afficher `manager_display_name` en liste / fiche.
3. Si **morale**, après création : afficher la checklist des documents lue dynamiquement (`requiredDocumentTypes` ou `documentsSummary.missingTypes`) et permettre l'upload (flux `signed-url` → `documents`).
4. Afficher un badge dossier (`documentsSummary.isComplete`) dans la fiche **et** la liste.
5. Requalifier les partenaires existants (tous `INDIVIDUAL`) en `COMPANY` via `PATCH` au cas par cas.
